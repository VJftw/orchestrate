...
def test(ctx):
    ...
    Docker.run(cli,
        ...
        volumes=[
            "{0}:/go/src/github.com/vjftw/orchestrate/cadet".format(os.getcwd())
        ],
        working_dir="/go/src/github.com/vjftw/orchestrate/cadet",
        environment={}
    )
    Docker.run(cli,
        tag="{0}-dev".format(flow.get_branch_container_name()),
...
def build(ctx):
    ...
    Docker.run(cli,
        tag="{0}-dev".format(flow.get_branch_container_name()),
        command='/bin/sh -c "go build -a -installsuffix cgo -o dist/cadet"',
        volumes=[
            "{0}:/go/src/github.com/vjftw/orchestrate/cadet".format(os.getcwd())
        ],
        working_dir="/go/src/github.com/vjftw/orchestrate/cadet",
